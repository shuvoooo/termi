...
`apps/web/src/proxy.ts` is the Next.js middleware that applies CSP and all security headers (not `middleware.ts`). Nonces are generated per-request and forwarded via the `x-nonce` request header to server components.
